...
export const ProcessoDetailsDrawer: React.FC<ProcessoDetailsDrawerProps> = ({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({ title: 'Processo atualizado', description: 'Capa salva com sucesso.' });
      setEditing(false);
    } catch (e: any) {
      ...
      if (andamentos.length > 0) {
        ...
        if (inseridos > 0) {
          toast({
            ...
            description: `${inseridos} nova(s) movimentação(ões) sincronizada(s).`,
          });
        }
  ...
  // Auto-sync ao abrir aba Histórico (1x por abertura)
  useEffect(() => {
  ...
  const EditableField = ({ label, field, icon }: { label: string; field: keyof typeof editData; icon?: React.ReactNode }) => (
    <div className="space-y-1">
      <p className="text-[10px] text-foreground/30 dark:text-muted-foreground/60 uppercase font-black tracking-widest">{label}</p>
      {editing ? (
        <Input
        ...
        <div className="flex items-center gap-2">
          {icon}
          <p className="text-sm font-bold text-foreground/80 dark:text-foreground">{String(editData[field]) || '—'}</p>
        </div>
      )}
    </div>
  );
          ...
          <div className="flex items-start justify-between relative">
            <div className="space-y-4 flex-1">
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-1.5 text-foreground/30 dark:text-muted-foreground/60 text-[10px] uppercase font-black tracking-widest">
              ...
              {editing ? (
                <Input
                  className="text-xl font-black rounded-xl bg-background border-border"
                  value={editData.titulo}
                  onChange={(e) => setEditData({ ...editData, titulo: e.target.value })}
                />
              ) : (
                <SheetTitle className="text-2xl md:text-3xl font-black text-foreground leading-tight tracking-tight">
                  {processo.titulo}
                </SheetTitle>
              )}
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              ...
              <TabsContent value="resumo" className="space-y-8 animate-in fade-in duration-300">
                  ...
                  <div className="p-5 rounded-[2rem] bg-emerald-500/5 border border-emerald-500/10 space-y-2">
                    ...
                    {editing ? (
                      <Input
                        type="number"
                        className="h-9 text-sm rounded-xl bg-background border-border"
                        value={editData.valor_causa}
                        onChange={(e) => setEditData({ ...editData, valor_causa: Number(e.target.value) || 0 })}
                      />
                    ) : (
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    {editing ? (
                      <div className="flex gap-2">
                        <Button size="sm" variant="ghost" onClick={() => setEditing(false)} className="h-8 rounded-xl text-xs">
                          <X className="h-3.5 w-3.5 mr-1" /> Cancelar
                        </Button>
                        <Button size="sm" onClick={handleSave} disabled={saving} className="h-8 rounded-xl text-xs">
                          <Save className="h-3.5 w-3.5 mr-1" /> {saving ? 'Salvando...' : 'Salvar'}
                        </Button>
                      </div>
                    ) : (
                      <Button size="sm" variant="outline" onClick={() => setEditing(true)} className="h-8 rounded-xl text-xs border-border">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 rounded-[2rem] border border-border bg-muted/30">
                    <EditableField label="Classe" field="classe_judicial" />
                    <EditableField label="Assunto Principal" field="assunto_principal" />
                    <EditableField label="Fase Atual" field="fase_processual" />
                    <EditableField label="Instância" field="instancia" />
                    <EditableField label="Tribunal" field="tribunal" icon={<Building2 className="h-3.5 w-3.5 text-primary/60" />} />
                    <EditableField label="Vara" field="vara" icon={<MapPin className="h-3.5 w-3.5 text-primary/60" />} />
                    <EditableField label="Comarca / UF" field="comarca" />
                  </div>
                </div>
              </TabsContent>
              ...
              <TabsContent value="timeline" className="space-y-6 animate-in fade-in duration-300">
                ...
                {loadingMovements && movements.length === 0 ? (
                  ...
                  <div className="py-32 flex flex-col items-center justify-center text-center space-y-6 opacity-30">
                    ...
                    <div className="space-y-2">
                      <p className="font-black uppercase tracking-widest">Sem Histórico</p>
              ...
              <TabsContent value="partes" className="space-y-6 animate-in fade-in duration-300">
                <div className="flex items-center justify-between -mt-2 mb-2">
                  ...
                  {!editing && (
                    <Button size="sm" variant="outline" onClick={() => { setEditing(true); setActiveTab('resumo'); }} className="h-8 rounded-xl text-xs border-border">
                      <Edit className="h-3.5 w-3.5 mr-1" /> Editar
                    </Button>
                  )}
                </div>
                ...
                <div className="space-y-6">
                  <div className="p-6 rounded-[2rem] border border-emerald-500/15 bg-emerald-500/5 space-y-3">
                    ...
                    {editing ? (
                      <Input
                        className="h-9 text-sm rounded-xl bg-background border-border"
                        value={editData.parte_autora}
                        onChange={(e) => setEditData({ ...editData, parte_autora: e.target.value })}
                  ...
                  <div className="p-6 rounded-[2rem] border border-rose-500/15 bg-rose-500/5 space-y-3">
                    ...
                    {editing ? (
                      <Input
                        className="h-9 text-sm rounded-xl bg-background border-border"
                        value={editData.requerido}
                        onChange={(e) => setEditData({ ...editData, requerido: e.target.value })}
